...
int main(int argc, char** argv) {
    printf("Тестирование cblas_zaxpy...\n");
    ...
    for (int i_n = 0; i_n < num_n; i_n++) {
        ...
        for (int i_inc = 0; i_inc < num_inc; i_inc++) {
            ...
            for (int i_alpha = 0; i_alpha < num_alphas; i_alpha++) {
                double complex alpha = alpha_values[i_alpha] + alpha_values[i_alpha]*I;
                // Вызов функции
                cblas_zaxpy(n, &alpha, (const double complex*)test_buffer, inc, test_buffer, inc);
            }
        }
    }
